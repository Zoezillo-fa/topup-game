...
class GameController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            // alpha_dash: hanya huruf, angka, dash, underscore (mencegah XSS di URL)
            'code' => 'required|string|unique:games,code|alpha_dash', 
            'slug' => 'required|string|unique:games,slug|alpha_dash',
        ...
        $data = $request->except(['thumbnail', 'banner']); // Ambil data teks saja
        
        // 2. UPLOAD THUMBNAIL (Secure Rename)
        if ($request->hasFile('thumbnail')) {
            $file = $request->file('thumbnail');
            // Rename file dengan string acak 40 karakter agar tidak bisa ditebak
            $filename = 'thumb_' . Str::random(40) . '.' . $file->getClientOriginalExtension();
            
            // Pindahkan ke folder public/images/games
        ...
        // 3. UPLOAD BANNER (Secure Rename)
        if ($request->hasFile('banner')) {
            $file = $request->file('banner');
            $filename = 'banner_' . Str::random(40) . '.' . $file->getClientOriginalExtension();
    ...
    public function destroy($id)
    {
        ...
        // Hapus file gambar fisik agar tidak menumpuk di server (Garbage Collection)
    ...
    public function syncDigiflazz()
    {
        ...
        $brands = collect($result['data'])
            ->filter(function ($item) {
                ...
                $catBlacklist = [
                    'pulsa', 'data', 'paket', 'internet', 'telepon', 'sms', 'roaming', 
                    'pln', 'listrik', 'token', 'tagihan', 'pascabayar', 
                    'pdam', 'air', 'gas', 'pertagas', 'pbb', 'samsat', 'pajak', 
                    'bpjs', 'asuransi', 'finance', 'multifinance', 'angsuran', 'kredit', 
                    'tv', 'televisi', 'internet', 'wifi', 'indihome', 
                    'e-money', 'emoney', 'wallet', 'saldo', 'topup', 
                    'dana', 'ovo', 'gopay', 'shopeepay', 'linkaja', 'sakuku', 'doku', 'isaku', 'jenius',
                    'voucher makanan', 'voucher belanja', 'transportasi', 'streaming', 'hiburan'
                ];
